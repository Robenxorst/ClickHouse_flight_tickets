-- Выполним несколько ad-hoc запросов для получения дополнительной информации;
-- Хотим добавить таблицу flight statuses, чтобы узнать о задержках рейсов (OTP - насколько рейс задержался);
...
select
    ...
    -- Процент рейсов, вылетевших вовремя. 
    -- Считаем, что рейс вылетел вовремя, если задержка не превышает 15 мин.
